    ...
    if (existingUser && !existingUser.password_hash && data.password) {
      ...
      const result = await sql`
        ...
        SET
          ...
          restaurant_id = COALESCE(${data.restaurant_id || null}, restaurant_id),
          location_id = COALESCE(${data.location_id || null}, location_id),
  ...
  static async update(userId: string, data: UpdateUserDto): Promise<User | null> {
    ...
    const result = await sql`
      ...
      SET
        ...
        restaurant_id = ${updated.restaurant_id || existing.restaurant_id},
        location_id = ${updated.location_id || existing.location_id},
